...
post '/trello/puppet-dev-community/?' do
  ...
  pr_num = gh_data['pull_request']['number']
  pr_url = gh_data['pull_request']['html_url']
  pr_body = gh_data['pull_request']['body']
  card_body = <<-EOBODY
Links: [Pull Request #{pr_number} Discussion](#{pr_url}) and
[File Diff](#{pr_url}/files)
#{pr_body}
  EOBODY
